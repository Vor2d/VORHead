...
public class GameController : GeneralGameController {
    ...
    public string MLHState
    {
        get 
        {
            return "|HL " + head_left.ToString() + "|LS " + MLH_left_stop_flag.ToString() +
                "|RS " + MLH_right_stop_flag.ToString() + "|CA " + curr_acuity_size.ToString() +
                "|MS " + MLHData.single_repeat_num.ToString() +
                "|MD " + MLHData.double_repeat_num.ToString() +
    ...
    private void check_speed_no_window()
    {
        if(Check_speed_flag)
        {
            ...
            if (head_speed_y < 0) { head_left = false; }
            if (head_speed_y > 0) { head_left = true; }
            if (Mathf.Abs(head_speed_y) > DC_script.SystemSetting.SpeedThreshold)
            {
                HeadLogSystem.IS.Add_game_state("Speed passed");
                ...
                speed_passed(speed: head_speed_y);
                return;
            }
    ...
    private void speed_passed(float speed = 0.0f)
    {
        head_timer.Start();
    ...
    private (bool, int) MLH_left_check_stop()
    {
        ...
        if (MLHLeftData.single_repeat_num > DataController.IS.SystemSetting.MLHSinRepeatNum)
        { return (true, curr_acuity_size); }
        if (MLHLeftData.double_repeat_num > DataController.IS.SystemSetting.MLHDouRepeatNum)
    ...
    private (bool, int) MLH_right_check_stop()
    {
        ...
        if (MLHRightData.single_repeat_num > DataController.IS.SystemSetting.MLHSinRepeatNum)
        { return (true, curr_acuity_size); }
        if (MLHRightData.double_repeat_num > DataController.IS.SystemSetting.MLHDouRepeatNum)
