...
public class StreamParser : MonoBehaviour
{
    ...
    //individual components in degrees of the quyaternion speed vectors
    ...
    void Start()
    {
        ...
        /*
        _thread1 = new Thread(() => VelocityThread2(plstream));
        _thread1.Start();
    ...
    */
        ...
        if (threadGo)
        {
            ...
            if (deltaTime >= .001f)
            {
                if (firstThreshold > 10)
                {
                    ...
                    /*
                     transfers dataStream translated bit information to unity engine parameters
                      */
                    dataPackage = dataStream.read();

                     //* Note: these two values currently unused
                    pol_position = dataStream.position - prime_position;
                    pol_rotation = dataStream.rotation //-calibrate_rotation;
                        ;

                    //current used versions of Position and Rotation
                    ...
                    motionx = (diffQuaternion.x * Mathf.Rad2Deg);
                    motiony = (diffQuaternion.y * Mathf.Rad2Deg);
                    motionz = (diffQuaternion.z * Mathf.Rad2Deg);
                    ...
                    avgMotionXQueue.Enqueue(motionx);
                    avgMotionYQueue.Enqueue(motiony);
                    avgMotionZQueue.Enqueue(motionz);

                    //collects history for 
                    if (velocityReadHistory.Count >= velocityHistorySize)
                        velocityReadHistory.Dequeue();
                    velocityReadHistory.Enqueue(new Vector3(motionx, motiony, motionz));
    ...
    {
        float accelx = (motionx - lastMotionX) / deltaTime;
        float accely = (motiony - lastMotionY) / deltaTime;
        float accelz = (motionz - lastMotionZ) / deltaTime;
    ...
    {
        assignMotion(ref thresholdX, ref lastMotionX, motionx);
        assignMotion(ref thresholdY, ref lastMotionY, motiony);
        assignMotion(ref thresholdZ, ref lastMotionZ, motionz);
    }
